...
def run_backtest(config: Any, repo: Any) -> dict[str, pd.DataFrame | dict[str, Any]]:
    ...
    if len(trading_dates_all) < 2:
        return {
            ...
            "summary": {"rebalances": 0, "final_equity": _pick_run_value(run_cfg, "initial_capital", default=1_000_000.0)},
        }
    ...
    state = PortfolioState(cash=float(_pick_run_value(run_cfg, "initial_capital", default=1_000_000.0)))
    ...
    summary = {
        ...
        "total_costs": float(pd.DataFrame(rebalance_records)["costs"].sum()) if rebalance_records else 0.0,
    }
